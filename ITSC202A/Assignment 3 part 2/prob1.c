...
int main ()
{
	...
	for(i=0; char2[i]; i++){ // "char2[i]" same as "/0 or null"
 	if(i==0){ // checking whenever i is equal to 0
			...
			if(char2[i] >= 'A' && char2[i] <= 'Z') // make it lowercase
				char2[i] = char2[i] + 32;
							...
							++i;
						if(char2[i] >= 'A' && char2[i] <= 'Z') // making first letter lowercase
							char2[i] = char2[i] + 32;
	...
	if(char1 == '2'){
	printf("*** Enter a string of length <= 40: ");
	scanf("%[^\n]s", char3); // checking for spaces
	for(j=0; j<sizeof(char3); j++){
		if(j % 2 == 0){ // whenever j is an even, itll be lowercase
				if(char3[j] >= 'a' && char3[j] <= 'z')
					char3[j] = char3[j] - 32;
						} else if (char3[j] >= 'A' && char3[j] <= 'Z'){
							char3[j] = char3[j] + 32;
							} else {
								char3[j] = char3[j]; // if its not  a letter, don't change anything
								}
	}
